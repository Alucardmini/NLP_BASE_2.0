...
from relationship_category.src import data_helpers
import time
import datetime
import os
import relationship_category.src.utils as utils
            ...
            with sess.as_default():
                ...
                for batch in batches:
                    ...
                    if step % FLAGS.evaluate_every == 0:
                        ...
                        time_str = datetime.datetime.now().isoformat()
                        f1 = f1_score(np.argmax(y_dev, axis=1), predictions, labels=np.array(range(1, 19)),
                                      average="macro")
                        print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
